...
myAditionalKeys =
  ...
  , ("M-v", spawn $ myTerminal ++ " --title config -e hx ~/.config/xmonad/xmonad.hs")
  ...
  -- screenshots
  , ("C-S-s", spawn "scrot -s - | xclip -selection clipboard -target image/png")
  , ("<Print>", spawn "scrot")
...
myStatusBarSpawner (S s) = do
                    pure $ statusBarPropTo ("_XMONAD_LOG_" ++ show s)
                          ("xmobar -x " ++ show s ++ " ~/.config/xmonad/xmobar" ++ show s ++ ".hs")
                          (pure $ myXmobarPP (S s))
